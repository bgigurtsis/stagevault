...
export default function RehearsalDetail() {
  ...
  return <div className="container py-6 space-y-8">
      ...
      <div className="flex flex-wrap gap-4 sm:gap-6 items-center">
        ...
        {rehearsal.location && <div className="flex items-center gap-2">
            <MapPin className="h-5 w-5 text-muted-foreground" />
            <span>{rehearsal.location}</span>
          </div>}
        {performance && <div className="flex items-center gap-2">
      ...
      <div>
        ...
        {recordings.length === 0 ? <Card>
            ...
            {recordings.map(recording => <Card key={recording.id} className="flex flex-col">
                ...
                <CardContent className="flex-grow">
                  {recording.thumbnailUrl ? <div className="aspect-video mb-3 overflow-hidden rounded-md bg-muted group relative cursor-pointer" onClick={() => openVideoDialog(recording)}>
                      <img src={recording.thumbnailUrl} alt={recording.title} className="h-full w-full object-cover" />
                      <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <Play className="h-12 w-12 text-white" />
                      </div>
                    </div> : <div className="aspect-video mb-3 overflow-hidden rounded-md bg-muted flex items-center justify-center cursor-pointer" onClick={() => recording.videoUrl ? openVideoDialog(recording) : null}>
                      <Video className="h-8 w-8 text-muted-foreground/50" />
                    </div>}
                  
                  {recording.notes && <p className="text-sm text-muted-foreground line-clamp-3 mb-2">
                      {recording.notes}
                    </p>}
                  
                  {recording.tags && recording.tags.length > 0 && <div className="flex flex-wrap gap-1 mt-2">
                      {recording.tags.map((tag, index) => <Badge key={index} variant="outline" className="text-xs">
                          {tag}
                        </Badge>)}
                    </div>}
                </CardContent>
                <CardFooter className="flex gap-2 pt-2 border-t">
                  <Button variant="outline" className="flex-1" onClick={() => openVideoDialog(recording)} disabled={!recording.videoUrl}>
                    <Play className="mr-2 h-4 w-4" /> Play
                  </Button>
                  {recording.videoUrl && <Button variant="outline" size="icon" asChild>
      ...
      <Dialog open={videoDialogOpen} onOpenChange={setVideoDialogOpen}>
        <DialogContent className="sm:max-w-[900px] p-0 overflow-hidden">
          ...
          <div className="aspect-video bg-black">
            {selectedRecording?.videoUrl && <iframe src={selectedRecording.videoUrl.replace('/view', '/preview')} className="w-full h-full" allow="autoplay; fullscreen" title={selectedRecording.title} />}
          </div>
          <div className="p-4">
            {selectedRecording?.notes && <div className="mb-4">
                <h4 className="text-sm font-medium mb-1">Notes:</h4>
                <p className="text-sm text-muted-foreground whitespace-pre-wrap">{selectedRecording.notes}</p>
              </div>}
            
            {selectedRecording?.tags && selectedRecording.tags.length > 0 && <div>
                <h4 className="text-sm font-medium mb-1">Tags:</h4>
                <div className="flex flex-wrap gap-1">
                  {selectedRecording.tags.map((tag, index) => <Badge key={index} variant="outline" className="text-xs">
                      {tag}
                    </Badge>)}
                </div>
              </div>}
            
            {selectedRecording?.videoUrl && <div className="mt-4 flex justify-end">
                <Button asChild>
